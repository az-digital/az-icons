...
: "${AZ_ICONS_FROZEN_DIR:=/azbuild/az-icons}"
...
if echo "$tagsearch" | grep -q "$oldhash" ; then
  ...
  imagenewid=$(docker build -q --build-arg AZ_ICONS_FROZEN_DIR . ) \
    || errorexit "Failed to build a new ${AZ_EPHEMERALIMAGENAME} Docker image preconfigured with the ${AZ_ICONS_FROZEN_DIR} npm directory"
